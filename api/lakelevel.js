export default async function handler(req, res) {
  ...
  try {
    ...
    if(!live){ try{const b=await fetchText('https://www.bizer.com/bzt/bztmap.shtml'); const m=b.match(/Lake\s*Level[^0-9]*([0-9]{3}\.[0-9]{1,2})/i); if(m){level=parseFloat(m[1]); time=new Date().toISOString(); live=true; source='Bizer.com';}}catch{}}
    if(!level){level=503.38; time='2017-09-30T00:45:00.000-04:00'; source='USGS 01080000 ended 2017-09-30 - see NH DES';}
    return res.json({level:Number(level.toFixed(2)),gageHeight:Number((level-500).toFixed(2)),time,source,fullPool:FULL_POOL,datumNote:'Lake level = gage height + 500ft datum',live,warning:!live?'USGS dead, check NH DES: https://www.des.nh.gov/water/dam-maintenance/dam-bureau/lake-levels':undefined});
